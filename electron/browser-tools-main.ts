...
const descriptions = {
  status:
    'Read browser connection state without connecting, prompting, or opening pages. Chromium rows identify extension profiles; "mako" is this host\'s hidden desk; desk rows with an origin and sourceRoot identify other active development hosts exactly.',
  connect:
...
Targets: one host-owned connection per browser or desk serves every task. For a development URL, choose the desk row whose origin and sourceRoot match it; that hidden window belongs to the development host and needs no Chromium connection or visible window. "mako" always means this task's own host and must not be used as a fallback for another profile. Connect the chosen ID if status shows it disconnected, then open a task-lifetime tab/window or inspect tabs and select an exact ID. Keep the returned {browser,tab,generation,lease} handle; every later call names it, there is no implicit active tab. Hidden-page input enables target-local focus emulation only for the duration of that action and disables it in cleanup; it never activates the physical page. A target-closed or stale-target error requires explicit rediscovery, never choosing the first available target.
...
The browser with ID "mako" is this host's Mako: open creates a hidden window of its own interface (1600×1000, larger than the screen if needed) that only this task sees. An active development host additionally appears as a distinct desk row carrying its exact origin and sourceRoot; use that row to inspect a Vite renderer from an installed-host task. Each desk enforces its own document policy, stays hidden, and never borrows another host's privileged preload. Close the tab when finished. Prefer a matching desk over computer tools whenever the target is Mako.
