...
import {
  HOST_CLOSED_CODE,
  HOST_RESTARTING_CODE,
  RuntimeDisconnectedError,
} from "./contracts/host-connection.js"
import {
  readRuntimeResponse,
...
export function requestRuntimePreview(
  socket: string,
  client: string,
  body: RuntimeCall,
  ...
  return new Promise<
    Awaited<ReturnType<typeof collectPreviewMedia>>["preview"]
  >((resolve, reject) => {
    const req = request(
      {
        socketPath: socket,
      ...
      (response) => {
        void (async () => {
          ...
          if (response.statusCode !== 200)
            throw new Error(`Mako preview returned ${response.statusCode}`)
          if (response.headers["content-type"] !== PREVIEW_MEDIA_TYPE) {
            const reply = RuntimeReplySchema.parse(
              JSON.parse((await readRuntimeResponse(response)).body)
            )
            if (!reply.ok) {
            ...
            throw new Error(
              "Preview delivery requires a matching Mako client and host. Existing agents have not been restarted."
            )
          }
          if (
          ...
          try {
            ...
            /* Diagnostics never change a successful read. */
          }
    ...
    req.setTimeout(5000, () =>
      req.destroy(new Error("Preview delivery timed out"))
    )
    req.on("error", reject)
