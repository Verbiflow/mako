...
export class WorkspaceGit {
  ...
  async remote(input: GitRemoteInput): Promise<GitRemoteResult> {
    ...
    if (failure instanceof KiriError && failure.code === "local_changes" && (input.action === "pull" || input.action === "merge")) return { status, problem: { kind: "dirty", message: "Local edits overlap incoming changes.", detail } }
