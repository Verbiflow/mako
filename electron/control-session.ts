...
export function createControlSession(
  ...
  const controlCapture = async (
    ...
    const parsedOptions = NativeScreenshotOptionsSchema.safeParse(
      request.options
    )
    if (!parsedOptions.success)
      throw new ControlFault(
        "invalid-request",
        `Invalid native screenshot options: ${z.prettifyError(parsedOptions.error)}. Use format, quality, maxSide or screenshot_out_file. Nothing was captured.`,
        "not-dispatched"
      )
    invalidateControlTarget(key)
    const captureOptions = parsedOptions.data
    ...
    const invoke = async () => {
      ...
      if (action === "connect") {
        if (!browserCall)
          throw new Error("Page control is unavailable outside a Mako task")
        return browserCall(
          BrowserCommandSchema.parse({ ...args, action: "connect" }),
          signal
        )
      }
      if (action === "targets") return controlTargets(args, signal)
      if (action === "observe" || action === "capture") {
        const target = ControlTargetSchema.parse(args.target)
        const key = controlTargetKey(target)
  ...
  return {
    ...
    async help(input: ComputerArguments = {}) {
      ...
      return z
        ...
        .parse(
          unified
            ? await controlHelp(controlHelpInputSchema.parse(input))
    ...
    async execute(
      ...
      assertOpen()
      const request = ControlProgramRequestSchema.parse(input)
