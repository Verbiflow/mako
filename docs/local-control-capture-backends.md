...
| [pixelflux](https://github.com/linuxserver/pixelflux/tree/40a9d46ba9b8c137b9812825041c68977ddf615f), `40a9d46` | Selkies's Rust/PyO3 capture/encoding dependency. Damage-aware X11/Wayland paths, software/GPU encoding, encoded recording sinks with bounded per-client queues and fragmented MP4 recording. MPL-2.0 repository; default encoder feature introduces separate GPL dependencies. | First smaller-component reuse investigation for the isolated Linux prototype and continuous recording. Published crate is a Python `cdylib`, not a drop-in Node/Rust package. Portal code selects monitors; exact-window Mako capture needs a different verified grant. Evaluate helper versus upstream library boundary and dependency cost. |
