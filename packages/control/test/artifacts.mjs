...
assert.match(
  artifactFileName("../../etc/passwd", "json"),
  /^passwd-[0-9a-f]{8}\.json$/
)
assert.match(artifactFileName("", "png"), /^artifact-[0-9a-f]{8}\.png$/)
const outline = outlineOf({
  elements: [{ role: "AXButton" }, { role: "AXLink" }],
  title: "x".repeat(300),
  n: 3,
  ok: true,
  none: null,
...
assert.deepEqual(
  outline.keys.map((k) => [k.name, k.type]),
  [
    ["elements", "array"],
    ["title", "string"],
    ["n", "number"],
...
try {
  const receipt = await spillJson(directory, "computer-result", {
    big: "y".repeat(50_000),
  })
  ...
  assert.equal(
    JSON.parse(await readFile(receipt.path, "utf8")).big.length,
    50_000
  )
