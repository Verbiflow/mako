...
export const HELPER_REFERENCE = [
  ...
  "act(action, args, {settle?, wait?, target?}) → {action, result, added: string[], removed: string[], unchanged: number}  One target-bound driver action (by name), then the lines that appeared and disappeared in that same window: read after settle ms (default 400) and re-read every 250 ms while nothing has changed, up to wait ms (default 2500). A click and what it revealed are one call; the new tokens are in added. An element_token is dispatched before act reads, so act cannot stale it itself. A pid or window_id that disagrees with target is refused.",
  ...
  "fill(element_token, text, {wait?, target?}) → {action, route, confirmed, line, view, result}  Text into a control without a keyboard: set_value, then the control read back until its line shows the text (confirmed) or wait ms (default 1500) pass. view is that exact newest snapshot; take the next action's token from it. Works on a backgrounded Electron, Chromium or Cocoa field; never fronts.",
  "submit(element_token, target?) → {action, route: confirm | press, result}  Enter without a keyboard: the control's confirm action, falling back to press only when the driver explicitly says confirm is unsupported. An unknown outcome is never repeated. Background.",
