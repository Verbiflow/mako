...
export const HELPER_REFERENCE = [
  ...
  "act(action, args, {settle?, wait?, target?, postcondition?}) → {action, target, opened, closed, postcondition, receipt, result, added, removed, unchanged}  Act, then independently observe. opened/closed report document-window changes. A postcondition(lines) is polled through wait and confirms the receipt; otherwise the receipt says changed or unchanged. act dispatches a token before reading, so it cannot stale that token.",
  ...
  "expect(boolean | predicate, message?)  Check a computed condition directly or an assumption against the current view; when it fails the program stops and the error carries the view, so a chained step never runs on a screen you did not expect.",
  "token(line) → string  Return the validated element token at the start of a line from view; never split or trim tokens by hand.",
  ...
  "fill(element_token, text, {wait?, target?}) → {action, route, confirmed, line, view, receipt, result}  Text into a control without a keyboard: set_value, then the control read back until its line shows the text (confirmed) or wait ms (default 1500) pass. view is that exact newest snapshot; take the next action's token from it. Works on a backgrounded Electron, Chromium or Cocoa field; never fronts.",
  "submit(element_token, target?) → {action, route: confirm | press, receipt, result}  Enter without a keyboard: the control's confirm action, falling back to press only when the driver explicitly says confirm is unsupported. An unknown outcome is never repeated. Background.",
  "routes(target?) → {target, routes}  Structured capabilities proven for this exact window: each command, page, accessibility, pixel, keyboard, menu and foreground route is available, unavailable or foreground-required and names its verification guarantee.",
  "route(intent, target?) → {status, capability | reasons}  Pick the strongest proven route for exact | text | control | page | pointer | keyboard | menu | visual intent. Foreground is returned as a requirement, never selected silently.",
] as const
