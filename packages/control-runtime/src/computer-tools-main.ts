      ...
      const detail = {
        ...
        message:
          error instanceof z.ZodError
            ? controlInputMessage(
                error,
                `arguments for ${request.params.name}`,
                request.params.name === execTool
                  ...
                  : "Use the arguments published by this tool’s help."
              )
        ...
        recovery: !dispatched
          ...
          : controlFaultData(error)?.code === "cell-pending" ||
            ...
            : controlFaultData(error)?.outcome === "not-dispatched"
              ? "This operation was not dispatched. Follow the correction in the message. Earlier steps in the program may have completed; do not replay the whole program."
