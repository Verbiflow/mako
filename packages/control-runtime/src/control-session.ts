      ...
      if (action === "recording") {
        const request = controlInput(
          z
            ...
            .safeParse(args),
          "recording request",
          "Use handle.record(options), then savedRecording.stop()/status(); CLI uses record stop/status --input recording.json."
        )
        if (request.target.kind === "window") {
