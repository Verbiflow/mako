...
import {applyControlEnvironment, controlLaunchInstructions} from "../electron/control-launch.js"
...
try {
  const launch=await sessions.start("binding",service.mint("conversation","binding"),async reason=>{
    reasons.push(reason)
    await service.revoke("conversation","binding")
  })
  const env: NodeJS.ProcessEnv={PATH:"/usr/bin",MAKO_CONTROL_URL:"must-not-leak",MAKO_CONTROL_TOKEN:"must-not-leak",MAKO_CONTROL_SESSION_FILE:"old"}
  ...
  while(fixture.targets.size>0 || reasons.length===0) {
    assert.ok(Date.now()<deadline,"Lost worker must release browser ownership")
    await delay(20)
  }
  ...
  console.log("Desktop owner: exact CLI environment without browser credentials, lost-worker browser cleanup, late-start refusal")
} finally {
  await sessions.close()
  await service.close()
  browsers.close()
