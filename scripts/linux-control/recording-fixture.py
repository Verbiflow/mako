...
area=Gtk.DrawingArea();area.set_size_request(640,300)
...
def pointer(kind):
    def receive(widget,event):
        ...
        state['points'].append({'kind':kind,'x':event.x,'y':event.y,'window_x':event.x+offset[0],'window_y':event.y+offset[1]})
