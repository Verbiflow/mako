...
try {
  ...
  // A stale target or unavailable remote must not become a dirty-worktree error.
  result = await workspace.remote({ cwd: local, branch: "main", head: "0".repeat(40), action: "pull" })
  assert.equal(result.problem?.kind, "failed")
  assert.match(result.problem?.detail ?? "", /HEAD or branch changed/)
  git(local, "remote", "set-url", "origin", join(root, "missing.git"))
  result = await act("pull")
  assert.equal(result.problem?.kind, "failed")
  assert.match(result.problem?.detail ?? "", /does not appear to be a git repository/)
  git(local, "remote", "set-url", "origin", remote)
  await commit(peer, "report.md", "incoming report\n"); git(peer, "push")
  await writeFile(join(local, "report.md"), "local report\n")
  const beforeBlocked = (await workspace.status()).head
  result = await act("merge")
  assert.equal(result.problem?.kind, "untracked")
  assert.equal(result.problem?.message, "A local file conflicts with incoming changes.")
  assert.match(result.problem?.detail ?? "", /report.md/)
  assert.equal(result.status.operation, undefined)
  assert.equal(result.status.head, beforeBlocked)
  assert.equal(await readFile(join(local, "report.md"), "utf8"), "local report\n")
  // Preserve the local copy under another name in this disposable fixture, then retry.
  await writeFile(join(local, "report.local.md"), await readFile(join(local, "report.md")))
  await rm(join(local, "report.md"))
  result = await act("merge")
  assert.equal(result.problem, undefined)
  assert.equal(await readFile(join(local, "report.local.md"), "utf8"), "local report\n")
  assert.equal(await readFile(join(local, "report.md"), "utf8"), "incoming report\n")
  await commit(peer, "shared.txt", "incoming overlap\n"); git(peer, "push")
  result = await act("pull")
  assert.equal(result.problem?.kind, "dirty")
  assert.match(result.problem?.detail ?? "", /shared.txt/)
  result = await act("merge_autostash")
  assert.equal(result.problem?.kind, "conflicts")
  assert.equal(result.status.operation, undefined)
  assert.equal(result.status.behind, 0)
